...
def read_velocities_from_file(filename):
    velocities = np.loadtxt(filename, delimiter=",")
    return velocities


def linear_and_angular_velo_opt_cpp():
    # Constants
    dt = 0.01  # Time step
    x_initial = 1
    y_initial = 5
    ...
    optimal_velocities = read_velocities_from_file("../build/nlopt_shooting_diffdrive.txt")
    optimal_velocities = optimal_velocities.T
    ...
    plot_trajectory(
        ...
        x_final,
        y_final,
        None,  # No final orientation specified
        x_values,
        y_values,
    )
...
def linear_and_angular_velo_opt():
    ...
    result = minimize(
        objective_function, initial_guess, method="SLSQP", options={"disp": True}
    )
...
def linear_and_angular_velo_all_final_specified():
    ...
    def objective_function(velocities):
        ...
        final_pose = poses[-1]
        terminal_position_cost = np.linalg.norm(
            final_pose[:2] - [x_final, y_final]
        )
...
def plot_trajectory(
    ...
    plt.quiver(
        x_initial,
        y_initial,
        1 * np.cos(theta_initial),
        1 * np.sin(theta_initial),
        color="g",
    )
    if theta_final:
        plt.quiver(
            x_final,
            y_final,
            1 * np.cos(theta_final),
            1 * np.sin(theta_final),
            color="r",
        )
    plt.xlabel("X")
    plt.ylabel("Y")
